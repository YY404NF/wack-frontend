...
export function useStudentState(deps: UseStudentStateDeps) {
  ...
  const managedClass = ref<ClassItem | null>(null)
  const managedClassStudents = ref<ClassStudentItem[]>([])
  const managedClassStudentsModalOpen = ref(false)
  const freeTimes = ref<FreeTimeEditorItem[]>([])
  ...
  const studentApp = useStudentApp({
    ...
    availableCourses,
    managedClass,
    managedClassStudents,
    managedClassStudentsModalOpen,
    freeTimes,
    freeTimeForm,
    freeTimeModalOpen,
    freeTimeTerm,
